...
# - Segmentation files with FS (T1 + FLAIR pial refinement)
...
# Reads T1/FLAIR pairs from a CSV (columns: t1w_filename, flair_filename)
# To test on a few subjects first, set N_TEST to a small number (e.g. 3),
# then set to 0 to run all.

# To run:
# /usr/bin/bash
# $ bash ./FreeSurfer_GT_FLAIR.sh

# ── Paths ─────────────────────────────────────────────────────────────────────
CSV_FILE="/analyse/Project0406/TheOneSeg/data/OpenMind/analyses/FLAIR_with_fullbrain_T1w.csv"
T1W_DIR="/analyse/Project0406/TheOneSeg/data/TheOneData/OpenMind/imgs/anat/T1w/"
FLAIR_DIR="/analyse/Project0406/TheOneSeg/data/TheOneData/OpenMind/imgs/anat/FLAIR/"

export OUT_DIR="/analyse/Project0406/TheOneSeg/data/TheOneData/OpenMind/imgs/derivative/FLAIR/"
export LOGS_DIR="/analyse/Project0406/TheOneSeg/data/OpenMind/T1_FLAIR_GT_test/logs/"
...
# ── Settings ──────────────────────────────────────────────────────────────────
N_THREADS=5           # number of subjects to run in parallel
export N_MULTI_OPENMP=3   # openmp threads per subject
export LEVEL_SETS_CAP_VALUE=5
N_TEST=0              # set to 0 to run all subjects; set to e.g. 3 to test first
...
# ── Read CSV into pairs ────────────────────────────────────────────────────────
# CSV columns: dataset,subject,session,t1w_filename,flair_filename,flair_is_full_brain
PAIRS=()
while IFS=',' read -r dataset subject session t1w_fname flair_fname flair_full; do
    [[ "$dataset" == "dataset" ]] && continue   # skip header
    [[ -z "$t1w_fname" || -z "$flair_fname" ]] && continue
    T1_PATH="${T1W_DIR}${t1w_fname}"
    FLAIR_PATH="${FLAIR_DIR}${flair_fname}"
    PAIRS+=("${T1_PATH}::${FLAIR_PATH}")
done < "$CSV_FILE"
...
gt_estimation_function(){
    ...
    if [ ! -d $I_OUT_TMP_FOLDER ]; then
        if [ ! -f $3 ]; then
            ...
            # Run FS recon-all with FLAIR pial refinement
            recon-all \
                -all \
...
grab_and_run_function(){
    # $1: "T1_PATH::FLAIR_PATH"
    T1_PATH="${1%%::*}"
    FLAIR_PATH="${1##*::}"

    I_SUBJ_FILENAME=$(basename "$FLAIR_PATH")
    I_SUBJ_FILENAME="${I_SUBJ_FILENAME%.nii.gz}"
    FULL_FILENAME_OUT="${OUT_DIR}${I_SUBJ_FILENAME}/${OUT_IDENTIFIER}"

    gt_estimation_function "$T1_PATH" "$FLAIR_PATH" "$FULL_FILENAME_OUT" "$I_SUBJ_FILENAME"
}
